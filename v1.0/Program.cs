...
class Program
{
    ...
    {
        List<int> invalidNumbers = new List<int> {169, 168, 166, 165, 163, 162, 159};
        ...
        while (true)
        {
            ...
            do
            {
                checkoutNumber = random.Next(2, 171);
            } while(invalidNumbers.Contains(checkoutNumber));

            // Suggest checkout
            Console.WriteLine($"Checkout suggestion for {checkoutNumber}:");
            SuggestCheckout(checkoutNumber);
    ...
    static void SuggestCheckout(int number)
    {
        if(number > 170)
        {
            Console.WriteLine("Impossible checkout.");
            return;
        }
